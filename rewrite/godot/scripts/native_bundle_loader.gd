...
func load_bundle(root: String, expected_key: String = "") -> Dictionary:
	var integrity = Integrity.new()
	var manifest: Dictionary = integrity.verify(root, expected_key)
	if manifest.is_empty():
		return {}
	var files := {}
	for entry: Dictionary in manifest["files"]:
		files[entry["path"]] = entry
	var chart: Dictionary = integrity.read_json(root.path_join("gameplay.json"), 67108864, files["gameplay.json"])
	var audio: Dictionary = integrity.read_json(root.path_join("audio-manifest.json"), 67108864, files["audio-manifest.json"])
	if not _chart_valid(chart) or not _audio_valid(audio, chart, manifest, files):
		return {}
	var mapping := {}
...
func _audio_valid(audio: Dictionary, chart: Dictionary, manifest: Dictionary, files: Dictionary) -> bool:
...
func _chart_valid(chart: Dictionary) -> bool:
