...
验收记录: nested TMPDIR 42 tests 零失败/跳过, golden gate 99 tests 零失败/跳过, Maven 全量 205 tests 零失败且有 8 个既有 skip. Rust release、version test、workspace gate 和 Godot import/smoke 可用; Rust all-targets test/clippy 仍因协议模块未实现失败, 归属 ticket 04. 固定 Godot 下载及 SDK 覆盖仅在项目本地生效. 当前等待 Standards/Spec 两轴审查, 不提前解除后续依赖.
