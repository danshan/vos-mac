...
func _init() -> void:
	...
	if not _expect_bool(view.load_metadata(metadata), true, "view metadata load"):
		return
	if not _expect_int(view.get_child_count(), 4, "view entity node count"):
		return
	if not _expect_bool(view.has_node("Entity_BGA"), true, "bga node"):
		return
	if not _expect_bool(view.has_node("Entity_NOTE_1"), true, "note node"):
		return

	var note_node: ColorRect = view.get_node("Entity_NOTE_1")
	if not _expect_float(note_node.position.x, 5.0, "note node x"):
		return
	if not _expect_float(note_node.position.y, 0.0, "note node y"):
		return
	if not _expect_float(note_node.size.x, 28.0, "note node width"):
		return
	if not _expect_float(note_node.size.y, 7.0, "note node height"):
		return
