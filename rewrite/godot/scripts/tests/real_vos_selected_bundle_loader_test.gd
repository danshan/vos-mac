...
const DEMO_CHART := "/Users/honghao.shan/Music/demo/Age of empire.vos"


func _init() -> void:
	if not FileAccess.file_exists(DEMO_CHART):
		print("Skipping real VOS selected bundle loader test; demo VOS file is not available.")
		quit(0)
		return
	...
	if not FileAccess.file_exists(jar_path):
		print("Skipping real VOS selected bundle loader test; packaged jar is not available.")
		quit(0)
		return

	var out_dir := ProjectSettings.globalize_path("user://real-vos-selected-export-%d" % Time.get_unix_time_from_system())
	...
	if not _expect_bool(bool(export_result.get("ok", false)), true, "selected VOS export"):
		return
	...
	if not _expect_bool(chart.is_empty(), false, "VOS gameplay load"):
		return
	if not _expect_string(str(chart.get("format", "")), "VOS", "VOS gameplay format"):
		return
	var notes: Array = chart.get("notes", [])
	if not _expect_bool(notes.size() > 0, true, "VOS gameplay notes"):
		return
	if not _expect_bool(notes.size() > 500, true, "VOS gameplay note volume"):
		return
	if not _expect_int(int(chart.get("keys", 0)), 7, "VOS key count"):
		return
	if not _expect_bool(int(notes[0].get("sampleId", 0)) > 0, true, "VOS note sample id"):
		return
	...
	if not _expect_bool(manifest.is_empty(), false, "VOS audio manifest load"):
		return
	if not _expect_string(str(manifest.get("format", "")), "VOS", "VOS audio format"):
		return

	var pool = AudioPlayerPool.new()
	get_root().add_child(pool)
	if not _expect_bool(pool.load_manifest(manifest), true, "VOS audio pool load"):
		return
	if not _expect_bool(pool.has_sample(1), true, "VOS sample asset"):
		return
