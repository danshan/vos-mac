...
func _init() -> void:
	...
	if not _verify_long_note_hold_miss(oracle):
		return
...
func _load_oracle() -> Dictionary:
	var path := "res://test/fixtures/long-note-hold-miss-oracle.json"
	if not FileAccess.file_exists(path):
		push_error("Missing long note hold miss oracle fixture: %s" % path)
		quit(1)
		return {}

	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(path))
	if not parsed is Dictionary:
		push_error("Expected long note hold miss oracle root object.")
		quit(1)
		return {}

	var root: Dictionary = parsed
	if int(root.get("schemaVersion", 0)) != 1:
		push_error("Expected long note hold miss oracle schema version 1.")
		quit(1)
		return {}
	if str(root.get("source", "")) != "Render.check_judgment LN_HOLD missed":
		push_error("Expected long note hold miss oracle source Render.check_judgment LN_HOLD missed.")
		quit(1)
		return {}
	return root


func _verify_long_note_hold_miss(oracle: Dictionary) -> bool:
	var controller = GameplayController.new()
	if not _expect_bool(controller.load_chart(_chart_from_oracle(oracle)), true, "oracle chart load"):
		return false
	...
	if not _expect_bool(head.get("accepted", false), true, "head accepted"):
		return false
	if not _expect_string(head.get("result", ""), "cool", "head result"):
		return false
	if not _expect_snapshot(controller, oracle.get("afterHead", {}), "after head", press_ms):
		return false

	var judged := controller.advance_to(float(oracle.get("checkMs", 0.0)))
	if not _expect_int(judged, 1, "hold miss judged count"):
		return false
	if not _expect_snapshot(controller, oracle.get("afterMiss", {}), "after miss", float(oracle.get("checkMs", 0.0))):
		return false
	return true
...
func _chart_from_oracle(oracle: Dictionary) -> Dictionary:
	return {
		"schemaVersion": 1,
		"chartId": "vos:long-note-hold-miss-oracle",
		"format": "VOS",
		"judgmentType": str(oracle.get("judgmentType", "time")),
		"rank": int(oracle.get("rank", 0)),
		"keys": 7,
		"bpm": 120.0,
		"durationMs": int(float(oracle.get("checkMs", 0.0)) + 1000.0),
		"notes": [
			{
